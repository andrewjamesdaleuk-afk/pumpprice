...
CREATE OR REPLACE FUNCTION get_premium_gap_by_brand()
RETURNS TABLE (
    brand VARCHAR,
    standard NUMERIC,
    premium NUMERIC,
...
BEGIN
    ...
    brand_averages AS (
        SELECT brand_name,
               AVG(CASE WHEN fuel_type IN ('E10', 'B7') THEN price END) AS avg_standard,
               AVG(CASE WHEN fuel_type IN ('E5', 'SDV') THEN price END) AS avg_premium,
               COUNT(CASE WHEN fuel_type IN ('E10', 'B7') THEN 1 END) AS count_standard,
               COUNT(CASE WHEN fuel_type IN ('E5', 'SDV') THEN 1 END) AS count_premium
        FROM current_prices
        GROUP BY brand_name
    )
...
GRANT EXECUTE ON FUNCTION public.get_premium_gap_by_brand() TO anon, authenticated;
