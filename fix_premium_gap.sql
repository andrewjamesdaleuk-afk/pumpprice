...
BEGIN
    IF target_fuel_type = 'E10' THEN
        -- Gov API mapping uses E5 instead of super petrol
        premium_type := 'E5';
    ELSE
        premium_type := 'B7_PREMIUM'; -- Wait, I mapped this in index.ts to 'SDV'!
    END IF;

    IF target_fuel_type = 'B7' THEN
        premium_type := 'SDV';
    END IF;
    ...
    latest_standard AS (
        ...
        FROM prices p
        WHERE fuel_type = target_fuel_type AND recorded_at > NOW() - INTERVAL '30 days'
        ORDER BY station_id, recorded_at DESC
    ),
    latest_premium AS (
        SELECT DISTINCT ON (station_id) station_id, p.price as item_price 
        FROM prices p
        WHERE fuel_type = premium_type AND recorded_at > NOW() - INTERVAL '30 days'
        ORDER BY station_id, recorded_at DESC
    ),
    brand_averages AS (
        SELECT c.brand_name,
    ...
    SELECT brand_name::VARCHAR as brand,
           (TRUNC(avg_standard) + 0.9)::NUMERIC as standard,
           (TRUNC(avg_premium) + 0.9)::NUMERIC as premium,
           ((TRUNC(avg_premium) + 0.9) - (TRUNC(avg_standard) + 0.9))::NUMERIC as gap
    FROM brand_averages
    WHERE count_standard > 20 AND count_premium > 10 AND avg_premium > avg_standard
