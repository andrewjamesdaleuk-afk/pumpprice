...
export function ExpensiveCitiesInsight() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const rankedCities = await fetchRankedCities('expensive', fuelType);
  ...
  return (
    <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden mt-6 mb-8">
      <div className="p-5 border-b border-slate-800">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-white">Most Expensive Cities</h3>
          </div>
        ...
        <p className="text-sm text-slate-400">
          The 3 most expensive UK cities for fuel. Ranking is based on the selected fuel's average price.
        </p>
      </div>
            ...
            {data.slice(0, 3).map((city, index) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs bg-red-500/20 text-red-400">
                    #{index + 1}
                  </div>
      ...
      <div className="p-5 border-t border-slate-800 bg-slate-800/20 text-sm text-slate-400 leading-relaxed">
        <h4 className="font-semibold text-slate-300 mb-2">Why is fuel more expensive in these cities?</h4>
        <p>
          Fuel prices in these top 3 most expensive UK cities often reflect higher local operating costs, reduced supermarket competition, and greater demand. Drivers in areas like London, Brighton, and other premium regions regularly see elevated petrol and diesel costs at the pump compared to the national average. Keeping an eye on localized price trends helps you avoid the most costly forecourts when planning your route.
        </p>
      </div>
    </div>
  );
