const fs = require('fs');
let code = fs.readFileSync('src/pages/LocalCity.tsx', 'utf8');
...
code = code.replace(
  `const prices = mappedResults.map((r: any) => r.price).filter((p: number) => p > 0);`,
  `const prices = mappedResults.filter((r: any) => !r.isStale).map((r: any) => r.price).filter((p: number) => p > 0);`
);
code = code.replace(
...
fs.writeFileSync('src/pages/LocalCity.tsx', code);
console.log('LocalCity.tsx updated!');
