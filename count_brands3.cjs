...
async function run() {
  const sql = `
    ...
    SELECT c.brand_name, COUNT(lf.item_price)::INT AS count_fuel
  ...
  console.log("Number of brands with >20 stations:", data.result.length);
  console.log(data.result.map(b => `${b.brand_name}: ${b.count_fuel}`).join(', '));
}
run();
