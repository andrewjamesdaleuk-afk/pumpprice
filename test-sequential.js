...
export const fetchCityStats = async (postcodeOrCity, bufferMeters = 8046.72) => {
  try {
    ...
    // SEQUENTIAL FETCH
    const petrolStations = await fetchStationsNearRoute(pointPolyline, 'petrol', bufferMeters);
    const dieselStations = await fetchStationsNearRoute(pointPolyline, 'diesel', bufferMeters);
