...
DROP FUNCTION IF EXISTS get_supermarket_sweep();
CREATE OR REPLACE FUNCTION get_supermarket_sweep()
RETURNS TABLE (
    region VARCHAR,
    petrolAvg NUMERIC,
    dieselAvg NUMERIC,
...
BEGIN
    ...
    region_averages AS (
        ...
        FROM major_cities c
        JOIN current_prices cp ON ST_DWithin(cp.location, c.geom, 50000) -- ~30 miles radius
...
GRANT EXECUTE ON FUNCTION public.get_supermarket_sweep() TO anon, authenticated;
