...
BEGIN
    ...
    city_stations AS (
        ...
        FROM stations s
        JOIN major_cities c ON ST_DWithin(s.location, c.geom, 1609.34)
    ),
    latest_prices AS (
        SELECT s.id, p.price AS fuel_price, p.recorded_at,
    ...
    current_prices AS (
        SELECT id, fuel_price FROM latest_prices WHERE rn = 1
    ),
    city_stats AS (
        SELECT 
            cs.city_name,
        ...
        JOIN current_prices cp ON cp.id = cs.id
