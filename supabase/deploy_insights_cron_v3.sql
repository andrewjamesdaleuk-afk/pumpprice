...
BEGIN
    ...
    -- 2. Premium Gap
    SELECT jsonb_agg(row_to_json(t)) INTO premium_gap_e10
    FROM (SELECT * FROM get_premium_gap_by_brand('E10')) t;

    SELECT jsonb_agg(row_to_json(t)) INTO premium_gap_b7
    FROM (SELECT * FROM get_premium_gap_by_brand('B7')) t;

    -- 3. Comprehensive City Stats (Calculating everything in one go)
